...
if (FC.ajaxurl.indexOf('?') > -1) { FC.ajaxurl2 = FC.ajaxurl+'&'; } else { FC.ajaxurl2 = FC.ajaxurl+'?'; }
...
function updateHelp(query, type, log)
{
	...
	.done(function(response) {
		...
		if (type=='categories')
		{
			...
			if ( jQuery('#help-menu ul').html().trim() == ''  ) { jQuery('#help-menu ul').html(html_li); }
			html = html + '</div>';	
		}
		else if (type=='posts')
		{
