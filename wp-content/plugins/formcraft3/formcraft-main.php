<?php
		...
		if(formcraft3_check_form_page())
		{
			...
			if(formcraft3_check_form_page_access($form_id))
			{
				...
				<head>';
					...
					if ( isset($qry['emailRecipients']) && $qry['emailRecipients']=='' )
					{
	...
	*/
	...
	function formcraft3_get_forms()
	{
		...
		{
			$total = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $fc_forms_table WHERE (name LIKE %s or id LIKE %s);", '%' . $wpdb->esc_like($_POST['query']) . '%', '%' . $wpdb->esc_like($_POST['query']) . '%') );			
			$forms = $wpdb->get_results( $wpdb->prepare( "SELECT id,name,modified FROM $fc_forms_table WHERE (name LIKE %s or id LIKE %s) $order_query LIMIT $from, $to;", '%' . $wpdb->esc_like($_POST['query']) . '%', '%' . $wpdb->esc_like($_POST['query']) . '%'), ARRAY_A );
		}
		else
		{
